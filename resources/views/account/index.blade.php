<x-layout title="Minha Conta">
    <div class="flex flex-col lg:flex-row justify-between mb-4">
        <div class="flex flex-wrap mb-2 lg:mb-0 gap-2 items-center justify-center">
            <a href="#" title="Trocar seu email"
                class="flex items-center ringo p-3 rounded-md text-white font-bold bg-yellow-800 hover:bg-yellow-900">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" />
                </svg>
                <span class="ml-3">Trocar Email</span>
            </a>

            <a href="#" title="Trocar sua senha"
                class="flex items-center ringo p-3 rounded-md text-white font-bold bg-gray-600 hover:bg-gray-700">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                </svg>
                <span class="ml-3">Trocar Senha</span>
            </a>
        </div>
        ...
        <div class="flex flex-wrap md:flex-nowrap gap-2 items-center justify-center">
            ...
            <form action="{{ route('logout') }}" method="post">
                ...
                <button title="Sair da conta"
                    class="ringo rounded-md text-white p-3 font-bold bg-red-700 hover:bg-red-800">
                    Sair
                </button>
            </form>
        </div>
    ...
    <x-box>
        ...
        <div class="grid gap-2">
            ...
            <div class="flex items-center">
                <a class="relative flex-1 bg-gray-900 hover:bg-gray-700 py-3 px-5 rounded-md"
                    href="{{ route('player.profile', ['name' => $player->name]) }}">
                    <span class="text-white font-bold">{{ $player->name }}</span>

                    <div class="bg-white absolute top-1/2 right-0 transform -translate-y-1/2 mr-4">
                        @if ($player->trashed())
                        <form action="{{ route('player.restore', ['id' => $player->id]) }}" method="post">
                            @csrf
                            <button title="Restaurar personagem" class="text-green-600 hover:text-green-200 font-bold">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <form action="{{ route('player.destroy', ['id' => $player->id]) }}" method="post">
                            ...
                            <button title="Deletar personagem temporariamente"
                                class="text-red-600 hover:text-red-200 font-bold">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
