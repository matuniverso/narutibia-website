<div class="flex flex-1 min-w-0" x-data="{ open: @entangle('isOpen').defer }">
  <button type="button" title="Pesquisa"
    ...
    x-on:click="open = !open; $nextTick(() => document.getElementById('searchInput').focus())">
    <svg
  ...
  <div x-show.transition.opacity="open" x-on:click="$wire.close()"
    ...
    <div class="flex flex-col w-11/12 md:w-7/12 mt-8">
      <x-input wire:model.debounce.350ms="search"
        x-on:keydown.escape="$wire.close()" type="search" name="search"
        id="searchInput" placeholder="Exemplo: Kakashi, Kage Quest"
        autocomplete="off" maxlength="50" />
      @if ($search)
        <div class="bg-white rounded-md p-2 mb-3 overflow-y-auto"
          x-on:click.stop>
          @if ($players->count() > 0 or $guilds->count() > 0)
            {{--  --}}
          @else
            <p class="text-gray-700 text-center">Não há resultados para "<span
                class="font-bold">{{ $search }}</span>"</p>
          @endif
        </div>
      @endif
      <button type="button" x-on:click.stop="$wire.close()"
